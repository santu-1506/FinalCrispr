...
class CRISPR_BERT:
    ...
    def forward(self, cnn_input, token_ids, segment_ids, position_ids, training=True):
        ...
        # Step 3: Separate BiGRU layers for each branch
        # CNN BiGRU: (batch, 26, 80) → (batch, 26, 80)
        # 40 forward + 40 backward = 80 (set return_sequences=True)
        cnn_gru_out = self.cnn_bigru.forward(cnn_features, return_sequences=True)  # (batch, 26, 80)
        print("CNN BiGRU output shape:", self.cnn_gru_out.shape)
        # BERT BiGRU: (batch, 26, 80) → (batch, 26, 80)
        # 40 forward + 40 backward = 80 (set return_sequences=True)
        bert_gru_out = self.bert_bigru.forward(bert_features, return_sequences=True)  # (batch, 26, 80)
        print("BERT BiGRU output shape:", self.bert_gru_out.shape)
        
        # Step 4: Weighted combination across full sequence
        # Both BiGRU outputs are (batch, 26, 80)
        cnn_weighted = self.w1 * cnn_gru_out   # (batch, 40)
        bert_weighted = self.w2 * bert_gru_out  # (batch, 40)

        # Weighted elementwise sum → (batch, 80)
        combined = cnn_weighted + bert_weighted

        # Step 6: Flatten sequence before Dense layers
        batch_size = combined.shape[0]
        flattened = combined.reshape(batch_size, -1)  # (batch, 26*80 = 2080)

        print("Combined BiGRU output shape:", self.combined.shape)
        print("Flattened shape:", self.flattened.shape)


        # Step 7: Dense layers with dropout
        x = self.dropout1.forward(flattened, training)
